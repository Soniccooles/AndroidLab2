...
object DataSource {
    fun getOddWeekSchedule(dayOfWeek: DayOfWeek): List<Lesson> {
    ...
    private fun getEvenWeekSchedule(dayOfWeek: DayOfWeek): List<Lesson> {
        return when (dayOfWeek) {
            DayOfWeek.MONDAY -> listOf(
                Lesson(
                    1,
    ...
    fun generateWeekSchedule(startDate: LocalDate, weekType: WeekType): List<Day> {
        ...
        for (i in 0 until 7) {
            ...
            val lessons = if (weekType == WeekType.ODD) {
                getOddWeekSchedule(dayOfWeek)
            } else {
                getEvenWeekSchedule(dayOfWeek)
            }
            weekDays.add(Day(dayOfWeek, date, lessons))
        }
        return weekDays
    }
